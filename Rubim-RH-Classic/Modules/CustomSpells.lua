...
RubimRH.queuedSpellAuto = { RubimRH.Spell[7].Default, 0 }
...
function RubimRH.QueuedSpell()
    return RubimRH.queuedSpell[1] or RubimRH.Spell[7].Default
end

function RubimRH.QueuedSpellAuto()
    return RubimRH.queuedSpellAuto[1] or RubimRH.Spell[7].Default
end
...
function Spell:IsQueuedPowerCheck(powerEx)
    ...
    if RubimRH.queuedSpell[1] == RubimRH.Spell[7].Default and RubimRH.queuedSpellAuto[1] == RubimRH.Spell[7].Default then
        return false
    end

    local powerCostQ, queuedSpellCD, queuedSpellID
    if RubimRH.queuedSpell[1] == RubimRH.Spell[7].Default then
...
function Spell:CooldownRemains(BypassRecovery, Offset)
    -- if RubimRH.db.profile[RubimRH.playerClass].Spells ~= nil then
        -- for i, v in pairs(RubimRH.db.profile[RubimRH.playerClass].Spells) do
...
HL:RegisterForSelfCombatEvent(function(_, _, _, _, _, _, _, _, _, _, _, SpellID)
    ...
    if RubimRH.QueuedSpellAuto().SpellID == SpellID then
        RubimRH.queuedSpellAuto = { RubimRH.Spell[2].Default, 0 }
    end

    if RubimRH.QueuedSpell().SpellID == SpellID then
        RubimRH.queuedSpell = { RubimRH.Spell[2].Default, 0 }
        RubimRH.print("|cFFFFFF00Queued:|r " .. GetSpellLink(SpellID) .. " casted!")
        RubimRH.playSoundR("Interface\\Addons\\Rubim-RH\\Media\\queuecast.ogg")
    end
    for i, spell in pairs(RubimRH.Spell[RubimRH.playerClass]) do
