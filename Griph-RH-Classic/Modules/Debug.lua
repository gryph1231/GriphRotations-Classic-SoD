...
if debugRub then
    ...
    UIDropDownMenu_Initialize(dropDown, function(self, level, menuList)
        ...
        if (level or 1) == 1 then
            ...
            info.func = function(self)
                PlaySound(891, "Master");
                local point, _, relativePoint, xOfs, yOfs = GriphRH.DebugFrame:GetPoint()
    ...
    GriphRH.DebugFrame = CreateFrame("Frame", "DebugFrame")
    GriphRH.DebugFrame:SetBackdrop(nil)
    GriphRH.DebugFrame:SetFrameStrata("TOOLTIP")
    GriphRH.DebugFrame:SetSize(50, 50)
    GriphRH.DebugFrame:SetScale(1);
    GriphRH.DebugFrame:SetPoint("CENTER", 0, 0)
    GriphRH.DebugFrame.texture = GriphRH.DebugFrame:CreateTexture(nil, "TOOLTIP")
    GriphRH.DebugFrame.texture:SetAllPoints(true)
    GriphRH.DebugFrame.texture:SetColorTexture(0, 1, 0, 1.0)

    GriphRH.DebugText = GriphRH.DebugFrame:CreateFontString("DebugText", "OVERLAY")
    GriphRH.DebugText:SetFontObject(GameFontNormalSmall)
    GriphRH.DebugText:SetJustifyH("RIGHT") --
    GriphRH.DebugText:SetPoint("TOP", GriphRH.DebugFrame, "BOTTOM", 100, -100)
    GriphRH.DebugText:SetFont("Fonts\\FRIZQT__.TTF", 20)
    GriphRH.DebugText:SetShadowOffset(1, -1)

    GriphRH.DebugFrame:SetMovable(true)
    GriphRH.DebugFrame:EnableMouse(true)
    GriphRH.DebugFrame:SetScript("OnMouseDown", function(self, button)
        if button == "LeftButton" and not self.isMoving then
            self:StartMoving();
            self.isMoving = true;
        end
    end)
    GriphRH.DebugFrame:SetScript("OnMouseUp", function(self, button)
        if button == "LeftButton" and self.isMoving then
            local arg1, _, arg2, arg3, arg4 = GriphRH.DebugFrame:GetPoint()
    ...
    GriphRH.DebugFrame:SetScript("OnHide", function(self)
    ...
    GriphRH.DebugFrame:SetScript("OnUpdate", function()
        --se tiver com problemas, colocar um delay usando o GetTime()
        --if t - GetTime() <= 0.2 then
        --    local point, _, relativePoint, xOfs, yOfs = GriphRH.DebugFrame:GetPoint()
        --    GriphRH.DebugText:SetText(point .. " , " .. relativePoint .. " , " .. xOfs .. " , " .. yOfs)
        --t = GetTime()
        --		end
    end)
end

--GriphRH.DebugFrame:Hide()
