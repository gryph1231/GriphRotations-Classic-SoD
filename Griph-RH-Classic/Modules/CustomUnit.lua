...
function GriphRH.UNITRange(unitID)
...
function Unit:LastDamage3Seconds()
    local IncomingDPS = (GriphRH.getLastDamage() / UnitHealthMax("player")) * 100
    return (math.floor((IncomingDPS * ((100) + 0.5)) / (100)))
end

function Unit:IncDmgPercentage()
    local unit = self.UnitID
    local IncomingDPS = (GriphRH.getDMG(unit) / UnitHealthMax(unit)) * 100
    return (math.floor((IncomingDPS * ((100) + 0.5)) / (100)))
end

function Unit:IncDmgSwing()
    local unit = self.UnitID
    local IncomingDPS = (GriphRH.incdmgswing(unit) / UnitHealthMax(unit)) * 100
    return (math.floor((IncomingDPS * ((100) + 0.5)) / (100)))
end

function Unit:LastSwinged()
    local unit = self.UnitID
    return GriphRH.lastSwing(unit)
end

-- Minor Defensive Usage (<= 1 Min CDs)
GriphRH.MinorHealingThreshold = 2
function Unit:NeedMinorHealing()
    return (self:IncDmgPercentage() > GriphRH.MinorHealingThreshold or self:HealthPercentage() <= 85)
end

-- Major Defensive Usage (<= 3 Min CDs)
GriphRH.MajorHealingThreshold = 5
function Unit:NeedMajorHealing()
    return (self:IncDmgPercentage() > GriphRH.MajorHealingThreshold or self:HealthPercentage() <= 60)
end

-- Panic Defensive Usage (> 3 Min CDs)
GriphRH.PanicHealingThreshold = 10
function Unit:NeedPanicHealing()
    return (self:IncDmgPercentage() > GriphRH.PanicHealingThreshold or self:HealthPercentage() <= 40)
end
...
function Unit:IsInterruptible()
    ...
    if spellId ~= nil then
        ...
        if GriphRH.db.profile.mainOption.whitelist then
            if GriphRH.db.profile.mainOption.interruptList[spellId] then
    ...
    if not GriphRH.InterruptsON() then
        return false
    end

    for i, v in pairs(GriphRH.db.profile.mainOption.interruptList) do
        if GriphRH.db.profile.mainOption.whitelist then
            return false
        end
    end
...
-- GriphRH.Listener:Add('Griph_Events', 'GROUP_ROSTER_UPDATE', function(...)
    -- CacheOtherTank()
-- end)
...
function Player:EnemiesAround(distance, ignoreCombat)
    local count = 0

    for k,v in pairs(HeroLibEx.Unit.Nameplate) do
        if v.UnitCanBeAttacked then
            local min, max = rc:GetRange(v.UnitID)
            if max <= distance then
                count = count + 1
            end
        end
    end
