...
function GriphRH.getRealTimeDMG(UNIT)
...
function GriphRH.getRealTimeDPS(UNIT)
    local total, Hits, phys, magic, swing = 0, 0, 0, 0, 0
    local combatTime = GriphRH.CombatTime(UNIT)
    local GUID = UnitGUID(UNIT)
...
function GriphRH.getHealSpellAmount(UNIT, SPELL, timer)
...
--[[ Get Heal Taken ]]
function GriphRH.getAbsorb(unit, spellID)
    local GUID = UnitGUID(unit)
...
function GriphRH.TimeToDieMagicX(UNIT, X)
    if not UNIT then 
		UNIT = "target" 
	end
...
function GriphRH.TimeToDieMagic(UNIT)
    if not UNIT then 
		UNIT = "target" 
	end
...
--- ========================== INCOMING ============================
-- DS 
function GriphRH.LastIncDMG(unit, seconds)
...
function GriphRH.LossOfControlCreate(locType, name, ...)
...
function GriphRH.LossOfControlRemove(locType, name)
    if name then 
...
function GriphRH.LossOfControlGet(locType, name)
    local result = 0
    if not LossOfControl[locType] then
...
-- function GriphRH.LossOfControlUpdate()
    ...
    -- if not isValidType then 
        -- for name in pairs(LossOfControl) do 
            -- if type(name) ~= "table" and GriphRH.LossOfControlGet(name) > 0 then
...
-- do 
    -- --- PvP Trinket:
    -- GriphRH.LossOfControlCreate("DISARM")
    -- GriphRH.LossOfControlCreate("INCAPACITATE")
    -- GriphRH.LossOfControlCreate("DISORIENT")
    -- GriphRH.LossOfControlCreate("FREEZE")        
    -- GriphRH.LossOfControlCreate("SILENCE")
    -- GriphRH.LossOfControlCreate("POSSESS")    
    -- GriphRH.LossOfControlCreate("SAP")    
    -- GriphRH.LossOfControlCreate("CYCLONE")
    -- GriphRH.LossOfControlCreate("BANISH")
    -- GriphRH.LossOfControlCreate("PACIFYSILENCE")
    -- --- Dworf|DarkIronDwarf
    -- GriphRH.LossOfControlCreate("POLYMORPH")    
    -- GriphRH.LossOfControlCreate("SLEEP")
    -- GriphRH.LossOfControlCreate("SHACKLE_UNDEAD")
    -- --- Scourge + WR Berserk Rage + DK Lichborne
    -- GriphRH.LossOfControlCreate("FEAR")    
    -- GriphRH.LossOfControlCreate("HORROR")    
    -- --- Scourge
    -- GriphRH.LossOfControlCreate("CHARM")        
    -- --- Gnome and any freedom effects 
    -- GriphRH.LossOfControlCreate("ROOT")        
    -- GriphRH.LossOfControlCreate("SNARE")
    -- --- Human + DK Icebound|Lichborne
    -- GriphRH.LossOfControlCreate("STUN")
-- end 

-- GriphRH.Listener:Add('Griph_Events', "LOSS_OF_CONTROL_UPDATE", GriphRH.LossOfControlUpdate)
-- GriphRH.Listener:Add('Griph_Events', "LOSS_OF_CONTROL_ADDED", GriphRH.LossOfControlUpdate)